...
function CustomerHomeContent({ user }) {
  ...
  const handleSearchAvailableRooms = async () => {
    if (!checkInDate || !checkOutDate) {
      setError("Vui lòng chọn ngày nhận và ngày trả phòng để tìm kiếm.");
      return;
    }

    if (checkOutDate < checkInDate) {
      setError("Ngày trả phòng phải sau hoặc bằng ngày nhận phòng.");
      return;
    }
        ...
        <section className="customer-featured" id="rooms">
          ...
          {!loading && error && (
            <div className="customer-status-box customer-status-box--error">
              Không thể tải dữ liệu phòng: {error}
            </div>
          )}

          {!loading && !error && featuredRooms.length === 0 && (
            <div className="customer-status-box">
              Chưa có loại phòng nào để hiển thị. Vui lòng thử lại sau.
            </div>
          )}
